...
class Packet(object):
    ...
    def __init__(self, doc=None, meta=None, attr_meta=None):
        ...
        if doc is None:
            ...
            doc = copy.deepcopy(dict(
                [(k_v[0], k_v[1]
                  if isinstance(k_v[1], list) else [k_v[1]])
                 for k_v in iter(doc.items())]))
    ...
    def get(self, attr, default=None):
        ...
        if attr in self._doc:
            ...
            if len(value) == 1 and not self.get_meta('multi', attr, False):
                value = value[0]
            else:
                value = value[:]
        else:
    ...
    def get_metas(self):
        ...
        return (copy.deepcopy(self._meta),
                copy.deepcopy(dict(self._attr_meta)))
    ...
    def set_meta(self, meta, value, attr=None):
        ...
        attr parameter.  This method will return False if the supplied
        attribute does not exist.
...
class PacketUnitTest(unittest.TestCase):
    ...
    def test_get_meta(self):
        ...
        self.assertEqual(self.doc.get_meta('am1', 'a'),
                         None, 'Failed get_meta')
    ...
    def test_has_meta(self):
        ...
        self.doc.set('a', 1)
        self.doc.set_meta('am1', 1, 'a')
        self.assertEqual(self.doc.has_meta('am1', 'a'),
                         True, 'Failed has_meta')
    ...
    def test_iter_eq_ne(self):
        ...
        self.doc.set('b', [4, 5])
        self.doc.set('c', 'test')
        self.assertEqual([x for x in self.doc],
                         [('a', [1, 2, 3]),
